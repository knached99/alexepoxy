...
export default function View({ auth, data }) {
    ...
    const editContent = async(values, {setSubmitting})=>{
        ...
        if(response.data.success){
            handleToast('success', resopnse.data.success);
        }
        else{
  ...
  return (
    <AuthenticatedLayout
      user={auth.user}
      header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Edit <span className="text-indigo-600">{data.photo_label}</span></h2>}
    >
      {data ? (
